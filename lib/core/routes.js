...
function routes(app) {
	...
	var initList = function(protect) {
		return function(req, res, next) {
			...
			if(!req.tenant){
				debug('could not find tenant ', req.params.tenant);
				req.flash('error', 'Tenant ' + req.params.tenant + ' could not be found.');
	...
	app.all('/keystone/:tenant/api/:list/:action', initList(), require('../../routes/api/list'));
