  ...
  debug('mounting');
	...
	if (this.get('mongo replica set')){
		...
		if(!_.isEmpty(this.multitenantMap)){
			var thisMongoose = this.mongoose;
			_.each(this.multitenantMap, function(mongo, tenant){
				debug('connecting to mongo tenant ' + tenant);
				//thisMongoose.createConnection(mongo);
			})
		}
	}
