...
List.prototype.register = function() {
	...
	var thisSchema = this.schema;
	_.each(keystone.multitenantMap, function(mongo, tenant){
		var match = mongo.match(dbNameRegx);
		if(!match.length){ return; }
		
		var dbName = match[0].replace("/", "");
		var conn = _.findWhere(keystone.mongoose.connections, { 'name': dbName});
		if(_.isUndefined(conn)){return;}
		
		conn.model(thisKey, thisSchema);
		models[tenant] = {
			conn: conn,
			dbName: dbName,
			model: conn.model(thisKey, thisSchema)
		}
	});	
	this.models = models;
